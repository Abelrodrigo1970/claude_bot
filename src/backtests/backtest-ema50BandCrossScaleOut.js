// Backtest da Ema50BandCrossScaleOut (src/strategies/ema50BandCrossScaleOut.js)
// sobre o universo atual do scanner EMA90 (top50-6month-movers usa um json
// fixo; aqui o universo é uma cópia do resultado real do scanner EMA90 em
// produção no momento em que este estudo foi feito — 03/09/2026 — filtrado
// dos contratos datados de futuros, que não são o par perpétuo normal).
//
// Gestão de posição igual à config de produção (runner.js): SL fixo 10%,
// TP1 +28%/30%, TP2 +48%/30% (cada fraction fecha % do que resta nesse
// momento), o resto sai quando o preço cai abaixo da EMA90 OU RSI(14)>87.
//
// Corre com: node src/backtests/backtest-ema50BandCrossScaleOut.js [dias]
...
// Snapshot do scanner EMA90 em produção, 03/09/2026 ~20:56 UTC (50
// resultados), sem os contratos datados de futuros do BTC (mantém só o
// perpétuo) — ver /api/scanner?period=90.
const SYMBOLS = [
...
// estratégia (entrada / saída por EMA90 ou RSI87).
function simulate(ticker, candles) {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    const { signal, indicators } = strat.generateSignal(window, pos ? 'long' : null);

    if (signal === 'long' && !pos) {
      pos = { entryPrice: price, entryTime: bar.time, qty: NOTIONAL / price, tpTierIndex: 0 };
    } else if (signal === 'close_long' && pos) {
      const tag = indicators.belowEma90 ? 'ema90-exit' : 'rsi87-exit';
...
async function main() {
  ...
  console.log('\n════════════════════════════════════════════════════════');
  console.log(`Ema50BandCrossScaleOut — ${DAYS} dias, universo EMA90, 4h, SL 10%, TP1 28%/30%, TP2 48%/30%, saída <EMA90 ou RSI>87`);
  console.log('════════════════════════════════════════════════════════');
